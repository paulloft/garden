...
//		}
     ...
     *  - <b>sql</b>: A SQLDriver with the child query.
    ...
    public static function Join(&$Data, $Columns, $Options = array()) {
        ...
        $JoinType = strtolower(val('Type', $Options, JOIN_LEFT));
        ...
        $ChildData = self::Index($ChildData, $ChildColumn, array('unique' => val('unique', $Options, isset($ColumnPrefix))));
            ...
            if (isset($ChildData[$ParentID])) {
                ...
                if (isset($ColumnPrefix)) {
                    ...
                    foreach ($ChildRow as $Name => $Value) {
                        setval($ColumnPrefix.$Name, $Row, $Value);
                    }
                } else {
                    // Add the result data.
                    setval($JoinColumn, $Row, $ChildRow);
                }
            } else {
                if ($JoinType == JOIN_LEFT) {
                    if (isset($ColumnPrefix)) {
                        foreach ($ResultColumns as $Name) {
                            setval($ColumnPrefix.$Name, $Row, NULL);
                        }
                    } else {
                        setval($JoinColumn, $Row, array());
                    }
                } else {
                    $NotFound[] = $Index;
                }
